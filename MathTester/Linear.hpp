...
public:
	...
	float GetDistance(Vector3& p) {
		Vector3 vec = p - position;  // p에서 포지션으로 가는 방향을 만들려면 p-position 반대로할경우 posi에서 p로가는방향의 벡터가 만들어짐
...
private:
	...
	float x, y, z, direction;  // ax*bx+ay*by+az*bz+d=0 일때 같음, -는 뒤로 +는 앞으로 떨어짐 D= (position|normalVector)의역 ==-(p|n)
public:
